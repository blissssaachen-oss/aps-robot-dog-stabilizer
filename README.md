# APS Robot Dog Stabilizer — Run & Logging Guide
...
## 3. To export (all) plot 
```bash
scp -r pi@<pi_ip_address>:~/team_say_alyssa/aps-robot-dog-stabilizer/log_walk .
```

# Stand test for IMU calib 
Test in P mode: Only launch Terminal A, C with logging
```bash
RCUTILS_COLORIZED_OUTPUT=0 python3 posture_stabalization.py 2>&1 | tee log_walk/stand_test_1.txt
grep "CSV," log_walk/stand_test_1.txt | sed 's/.*CSV,//' > log_walk/stand_test_1.csv
